...
/** One of the household's four dogs — the profile card shown at the
 * no-login /dog-sitter page and edited by an admin at /admin/dogs. */
export interface Dog {
  id: UUID;
  /** Stable identity ('dog-1'..'dog-4') independent of the dog's name. */
  slug: string;
  name: string;
  /** A data URL (photo uploaded via the admin form) or a hosted image URL. */
  photo: string | null;
  breed: string | null;
  age: string | null;
  weight: string | null;
