      ...
      {!code && (
        <>
          ...
          {isChoiceTier ? (
            !canRedeemNow && (
              <ul className="flex flex-col gap-1">
                {tier.choice_options!.map((option) => (
                  <li key={option} className="flex items-start gap-1.5 text-sm text-[var(--color-charcoal)]/70">
            ...
            <p className="flex items-start gap-1.5 text-sm text-[var(--color-charcoal)]/70">
              <Check size={13} className="mt-0.5 shrink-0 text-[var(--color-gold)]" />
              {hasSubscriberBoost
                ? `${tier.discount_percent}% off food & merch — ${tier.subscriber_discount_percent}% for subscribers`
          ...
          {canRedeemNow ? (
            isChoiceTier ? (
              <div className="flex flex-col gap-2">
                <p className="text-xs text-[var(--color-charcoal)]/55">Choose your reward:</p>
                {tier.choice_options!.map((option) => (
                  <Button
                    key={option}
                    variant="outline"
          ...
          {!isSubscriber && hasSubscriberBoost && (
            <Link
